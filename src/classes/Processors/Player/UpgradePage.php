...
class UpgradePage extends BaseProcessor
{
    ...
    public function Process(): ResultData
    {
        ...
        foreach ($upgradeData as $item) {
            ...
            $itemData[$item->ItemID]['autoAmount'] = $costAmount;
        }
        ...
        foreach ($upgradeData as $item) 
        {
            if ($expToCost <= 0) continue;
            $amount = $userBagHandler->GetItemAmount($item->ItemID);
            if ($amount >= $itemData[$item->ItemID]['autoAmount'] + 1) {
                $itemData[$item->ItemID]['autoAmount']++;
                $expToCost -= $item->EffectValue;
            }
        }
        if ($expToCost < 0) {
            $diff = -$expToCost;
            foreach ($upgradeData as $item) {
                $delAmount = floor($diff / $item->EffectValue);
                $itemData[$item->ItemID]['autoAmount'] = max($itemData[$item->ItemID]['autoAmount'] - $delAmount,0);
            }
        }
